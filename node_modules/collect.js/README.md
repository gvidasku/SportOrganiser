...
collection.contains('name', 'Steve sportevents');
// false
```
...
const collection = collect({
  name: 'Elon Musk',
  organisators: ['Tesla', 'Space X', 'SolarCity'],
});
...
> When working with nested objects `where()` method allows dot notated keys. E.g. `where('product.city', 'office-supplies')`
...
> When working with nested objects `whereIn()` method allows dot notated keys. E.g. `whereIn('product.city', ['office-supplies', 'furniture'])`
...
> When working with nested objects `whereNotIn()` method allows dot notated keys. E.g. `whereNotIn('product .city', ['office-supplies', 'furniture'])`
