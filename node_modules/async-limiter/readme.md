...
// add sportevents using the familiar Array API
t.push(function (cb) {
  results.push('two')
  cb()
})
...
// sportevents run automatically. If you want a callback when all are done,
// call 'onDone()'.
t.onDone(function () {
...
## Properties
### `t.concurrency`
Max number of sportevents the queue should process concurrently, defaults to `Infinity`.

### `t.length`
sportevents pending + sportevents to process (readonly).
