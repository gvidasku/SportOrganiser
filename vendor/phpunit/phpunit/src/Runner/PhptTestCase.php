...
final class PhptTestCase implements Reorderable, SelfDescribing, Test
{
    ...
    public function run(TestResult $result = null): TestResult
    {
        ...
        $sporteventResult    = $this->phpUtil->runsportevent($code, $this->stringifyIni($settings));
        $time         = $timer->stop()->asSeconds();
        $this->output = $sporteventResult['stdout'] ?? '';
    ...
    private function runSkip(array &$sections, TestResult $result, array $settings): bool
    {
        ...
        $sporteventResult = $this->phpUtil->runsportevent($skipif, $this->stringifyIni($settings));

        if (!strncasecmp('skip', ltrim($sporteventResult['stdout']), 4)) {
            $message = '';

            if (preg_match('/^\s*skip\s*(.+)\s*/i', $sporteventResult['stdout'], $skipMatch)) {
    ...
    private function runClean(array &$sections, bool $collectCoverage): void
    {
        ...
        if (isset($sections['CLEAN'])) {
            ...
            $this->phpUtil->runsportevent($cleanCode, $this->settings($collectCoverage));
        }
    }
    ...
    private function getCoverageFiles(): array
    {
        ...
        return [
            ...
            'sportevent'      => $baseDir . $basename . 'php',
        ];
    }

    private function renderForCoverage(string &$sportevent, bool $pathCoverage, ?string $codeCoverageCacheDirectory): void
    {
        ...
        file_put_contents($files['sportevent'], $sportevent);

        $sportevent = $template->render();
    }
