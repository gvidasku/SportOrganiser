...
trait MocksApplicationServices
{
    ...
    /**
     * All of the dispatched sportevents.
     *
     * @var array
     */
    protected $dispatchedsportevents = [];
    ...
    /**
     * Specify a list of sportevents that should be dispatched for the given operation.
     *
     * These sportevents will be mocked, so that handlers will not actually be executed.
     *
     * @param  array|string  $sportevents
     * @return $this
     */
    protected function expectssportevents($sportevents)
    {
        $sportevents = is_array($sportevents) ? $sportevents : func_get_args();

        $this->withoutsportevents();

        $this->beforeApplicationDestroyed(function () use ($sportevents) {
            $dispatched = $this->getDispatchedsportevents($sportevents);

            $this->assertEmpty(
                $sporteventsNotDispatched = array_diff($sportevents, $dispatched),
                'These expected sportevents were not dispatched: ['.implode(', ', $sporteventsNotDispatched).']'
            );
        });
    ...
    /**
     * Specify a list of sportevents that should not be dispatched for the given operation.
     *
     * These sportevents will be mocked, so that handlers will not actually be executed.
     *
     * @param  array|string  $sportevents
     * @return $this
     */
    protected function doesntExpectsportevents($sportevents)
    {
        $sportevents = is_array($sportevents) ? $sportevents : func_get_args();
        ...
        $this->beforeApplicationDestroyed(function () use ($sportevents) {
            $this->assertEmpty(
                $dispatched = $this->getDispatchedsportevents($sportevents),
                'These unexpected sportevents were dispatched: ['.implode(', ', $dispatched).']'
            );
        });
    ...
    /**
     * Mock the sportevent dispatcher so all sportevents are silenced and collected.
     *
     * @return $this
     */
    protected function withoutsportevents()
    {
        ...
        $mock->shouldReceive('dispatch', 'dispatchNow')->andReturnUsing(function ($dispatched) {
            $this->dispatchedsportevents[] = $dispatched;
        });
    ...
    /**
     * Filter the given sportevents against the dispatched sportevents.
     *
     * @param  array  $sportevents
     * @return array
     */
    protected function getDispatchedsportevents(array $sportevents)
    {
        return $this->getDispatched($sportevents, $this->dispatchedsportevents);
    }
