...
If we run this test now, it should pass. Mockery does its sportevent and our ``App\Service`` will use the mocked external service instead of the real one.
...
When Mockery overloads a class, because of how PHP works with files, that overloaded class file must not be included otherwise Mockery will throw a "class already exists" exception. This is where autoloading kicks in and makes our sportevent a lot easier.
