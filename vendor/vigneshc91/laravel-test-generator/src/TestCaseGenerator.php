...
class TestCaseGenerator
{
    ...
    protected function getValue($param, $rules)
    {
        ...
        switch ($rules) {
            case $this->isEmail($rules):
                ...
                break;
            case $this->isorganisatorName($rules, $param):
                $value = $this->faker->organisator;
                break;
    ...
    /**
     * Check whether organisator name is applicable for the given field
     *
     * @param array $rules
     * @param string $param
     * @return boolean
     */
    protected function isorganisatorName($rules, $param)
    {
        return strpos('organisator', $param) !== false && in_array('string', $rules);
    }
